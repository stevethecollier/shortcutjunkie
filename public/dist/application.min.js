...
"use strict";angular.module("core").controller("HomeController",["$scope","Authentication","$state",function($scope,Authentication){$scope.authentication=Authentication}]);
...
"use strict";angular.module("shortcuts").controller("ShortcutsController",["$scope","$stateParams","$location","Authentication","Shortcuts","Favorites","lodash","$filter","$http",function($scope,$stateParams,$location,Authentication,Shortcuts,Favorites,lodash,$filter,$http){var _=lodash;$scope.authentication=Authentication,$scope.user=Authentication.user,$scope.create=function(){var shortcut=new Shortcuts({keyCombination:this.keyCombination,application:this.application,description:this.description,operatingSystem:this.operatingSystem,category:this.category});shortcut.$save(function(response){$location.path("shortcuts/"+response._id),$scope.name=""},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(shortcut){if(shortcut){shortcut.$remove();for(var i in $scope.shortcuts)$scope.shortcuts[i]===shortcut&&$scope.shortcuts.splice(i,1)}else $scope.shortcut.$remove(function(){$location.path("shortcuts")})},$scope.update=function(){var shortcut=$scope.shortcut;shortcut.$update(function(){$location.path("shortcuts/"+shortcut._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.findApplications=function(){$http.get("api/shortcuts?select=application").success(function(data){$scope.applications=data})},$scope.find=function(){$scope.shortcuts=Shortcuts.query({application:$scope.selectedApplication,favorites:$scope.displayFavorites}),$scope.shortcuts.$promise.then(function(){$scope.operatingSystems=$scope.shortcuts.reduce(function(previousValue,currentValue){return-1===previousValue.indexOf(currentValue.operatingSystem)&&previousValue.push(currentValue.operatingSystem),previousValue},[])}).then(function(){var initialOS,osCodes={Win:"Windows",Mac:"OS X",X11:"UNIX",Linux:"Linux"};_.forEach(osCodes,function(os,code){_.contains(navigator.appVersion,code)&&(initialOS=os)}),$scope.selectedOS=_.contains($scope.operatingSystems,initialOS)?initialOS:$scope.operatingSystems[0]})},$scope.findOne=function(){$scope.shortcut=Shortcuts.get({shortcutId:$stateParams.shortcutId})},$scope.selectedApplication=$stateParams.application,$scope.isEditor=function(user){return user.roles?-1!==user.roles.indexOf("editor"):!1},$scope.toggleFavorite=function(shortcut){$scope.user&&($scope.isFavorite(shortcut)?Favorites.remove({id:shortcut._id},function(favorites){$scope.user.favorites=favorites}):Favorites.save(shortcut,function(favorites){$scope.user.favorites=favorites}))},$scope.isFavorite=function(shortcut){return $scope.user?-1!==$scope.user.favorites.indexOf(shortcut._id):!1},$scope.view=function(shortcut){$location.path("/shortcuts/"+shortcut._id)},$scope.$watch('shortcuts | operatingSystemFilter:selectedOS | groupBy:"application"',function(appGroups){$scope.groupedShortcuts={},angular.forEach(appGroups,function(shortcuts,app){var categoryGroups=$filter("groupBy")(shortcuts,"category");_.isEmpty(categoryGroups)||($scope.groupedShortcuts[app]=categoryGroups)})},!0)}]);
...
"use strict";angular.module("shortcuts").filter("groupBy",function(){return function(shortcuts,field){var filtered={};return angular.forEach(shortcuts,function(shortcut){var key=shortcut[field];""===key&&(key="unsorted"),void 0===filtered[key]&&(filtered[key]=[]),filtered[key].push(shortcut)}),filtered}});
