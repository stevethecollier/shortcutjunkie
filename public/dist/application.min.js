...
"use strict";angular.module("shortcuts").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Shortcuts",state:"shortcuts",type:"dropdown",isPublic:!0}),Menus.addSubMenuItem("topbar","shortcuts",{title:"Default Shortcuts",state:"shortcuts.applications",isPublic:!0}),Menus.addSubMenuItem("topbar","shortcuts",{title:"Create Shortcut",state:"shortcuts.create",isPublic:!1,roles:["editor"]}),Menus.addSubMenuItem("topbar","shortcuts",{title:"View your favorite shortcuts",state:"shortcuts.favorites",isPublic:!1})}]);
"use strict";angular.module("shortcuts").config(["$stateProvider",function($stateProvider){$stateProvider.state("home.applications",{views:{shortcuts:{templateUrl:"modules/shortcuts/views/applications-shortcuts.client.view.html"}}}).state("shortcuts",{"abstract":!0,url:"/shortcuts",template:"<ui-view/>"}).state("shortcuts.applications",{url:"/application/",templateUrl:"modules/shortcuts/views/applications-shortcuts.client.view.html"}).state("shortcuts.application",{url:"/application/:application",templateUrl:"modules/shortcuts/views/list-shortcuts.client.view.html"}).state("shortcuts.favorites",{url:"/favorites",templateUrl:"modules/shortcuts/views/list-shortcuts.client.view.html",resolve:{displayFavorites:function(){return{value:!0}}},controller:["$scope","displayFavorites",function($scope,displayFavorites){$scope.displayFavorites=displayFavorites.value}]}).state("shortcuts.create",{url:"/create",templateUrl:"modules/shortcuts/views/create-shortcut.client.view.html"}).state("shortcuts.view",{url:"/:shortcutId",templateUrl:"modules/shortcuts/views/view-shortcut.client.view.html"}).state("shortcuts.edit",{url:"/:shortcutId/edit",templateUrl:"modules/shortcuts/views/edit-shortcut.client.view.html"})}]);
"use strict";angular.module("shortcuts").controller("ShortcutsController",["$scope","$stateParams","$location","Authentication","Shortcuts","Favorites","lodash","$filter",function($scope,$stateParams,$location,Authentication,Shortcuts,Favorites,lodash,$filter){var _=lodash;$scope.authentication=Authentication,$scope.user=Authentication.user,$scope.create=function(){var shortcut=new Shortcuts({keyCombination:this.keyCombination,application:this.application,description:this.description,operatingSystem:this.operatingSystem,category:this.category});shortcut.$save(function(response){$location.path("shortcuts/"+response._id),$scope.name=""},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(shortcut){if(shortcut){shortcut.$remove();for(var i in $scope.shortcuts)$scope.shortcuts[i]===shortcut&&$scope.shortcuts.splice(i,1)}else $scope.shortcut.$remove(function(){$location.path("shortcuts")})},$scope.update=function(){var shortcut=$scope.shortcut;shortcut.$update(function(){$location.path("shortcuts/"+shortcut._id)},function(errorResponse){$scope.error=errorResponse.data.message})};var pickInitialOS=function(operatingSystems){var initialOS="";return-1!==navigator.appVersion.indexOf("Win")?initialOS="Windows":-1!==navigator.appVersion.indexOf("Mac")?initialOS="OS X":-1!==navigator.appVersion.indexOf("X11")?initialOS="UNIX":-1!==navigator.appVersion.indexOf("Linux")&&(initialOS="Linux"),-1!==operatingSystems.indexOf(initialOS)?initialOS:operatingSystems[0]};$scope.find=function(){$scope.shortcuts=Shortcuts.query({application:$scope.selectedApplication,favorites:$scope.displayFavorites}),$scope.shortcuts.$promise.then(function(){$scope.applications=$scope.shortcuts.reduce(function(previousValue,currentValue){return-1===previousValue.indexOf(currentValue.application)&&previousValue.push(currentValue.application),previousValue},[]),$scope.operatingSystems=$filter("applicationFilter")($scope.shortcuts,$scope.selectedApplication).reduce(function(previousValue,currentValue){return-1===previousValue.indexOf(currentValue.operatingSystem)&&previousValue.push(currentValue.operatingSystem),previousValue},[])}).then(function(){$scope.selectedOS=pickInitialOS($scope.operatingSystems)})},$scope.findOne=function(){$scope.shortcut=Shortcuts.get({shortcutId:$stateParams.shortcutId})},$scope.selectedApplication=$stateParams.application,$scope.isEditor=function(user){return user.roles?-1!==user.roles.indexOf("editor"):!1},$scope.toggleFavorite=function(shortcut){$scope.user&&($scope.isFavorite(shortcut)?Favorites.remove({id:shortcut._id},function(favorites){$scope.user.favorites=favorites}):Favorites.save(shortcut,function(favorites){$scope.user.favorites=favorites}))},$scope.isFavorite=function(shortcut){return $scope.user?-1!==$scope.user.favorites.indexOf(shortcut._id):!1},$scope.view=function(shortcut){$location.path("/shortcuts/"+shortcut._id)},$scope.$watch('shortcuts | applicationFilter:selectedApplication | operatingSystemFilter:selectedOS | groupBy:"application"',function(appGroups){$scope.appGroups={},angular.forEach(appGroups,function(shortcuts,app){$scope.displayFavorites&&(shortcuts=_.remove(shortcuts,function(shortcut){return _.contains($scope.user.favorites,shortcut._id)}));var categoryGroup=$filter("groupBy")(shortcuts,"category");_.isEmpty(categoryGroup)||($scope.appGroups[app]=categoryGroup)})},!0)}]);
...
"use strict";angular.module("shortcuts").filter("groupBy",function(){return function(shortcuts,field){var filtered={};return angular.forEach(shortcuts,function(shortcut){var value=shortcut[field];""===value&&(value="unsorted"),void 0===filtered[value]&&(filtered[value]=[]),filtered[value].push(shortcut)}),filtered}});
